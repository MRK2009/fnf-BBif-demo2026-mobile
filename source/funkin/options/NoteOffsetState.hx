...
class NoteOffsetState extends MusicBeatState
{
	...
	{
		// data todo make the combos redundant because im changing how they are positioned.
		// Cameras
		...
		new funkin.states.stages.BasicClassroom();
		
		// Characters
		gf = new Character(-100, 150, 'gf');
		gf.x += gf.positionArray[0];
		gf.y += gf.positionArray[1];
		gf.scrollFactor.set(0.95, 0.95);
		boyfriend = new Character(1100, 10, 'bf', true);
		...
		// Note delay stuff
		beatText = new BaldiText(0, 0, 'Beat Hit!', 48);
		// beatText.setScale(0.6, 0.6);
		beatText.borderStyle = OUTLINE;
		beatText.borderSize = 2;
		beatText.moves = true;
		beatText.x += 260;
		beatText.alpha = 0;
		...
		changeModeText.setFormat(Paths.font("comic.ttf"), 32, FlxColor.WHITE, CENTER);
	...
	override public function update(elapsed:Float)
	{
		...
		if (controls.BACK)
		{
			...
			FlxG.switchState(() -> new funkin.options.OptionsState());
			if (OptionsState.onPlayState)
			{
				if (!ClientPrefs.data.pauseMusic) FlxG.sound.playMusic(Paths.music(Constants.PAUSE_MUSIC));
				else FlxG.sound.music.volume = 0;
			}
			else FlxG.sound.playMusic(Paths.music(Constants.MENU_MUSIC));
	...
	override public function beatHit()
	{
		...
		if (curBeat % 4 == 2)
		{
			...
			if (zoomTween != null) zoomTween.cancel();
			zoomTween = FlxTween.tween(FlxG.camera, {zoom: 0.65}, 1,
				{
