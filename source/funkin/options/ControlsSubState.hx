...
class ControlsSubState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		grpDisplay = new FlxTypedGroup<OptionFlxText>();
		add(grpDisplay);
		...
		add(selectSpr);
		grpBinds = new FlxTypedGroup<OptionFlxText>();
		add(grpBinds);
		...
		var text:OptionFlxText = new OptionFlxText(65, 100, 'CTRL', 48);
		text.color = FlxColor.WHITE;
		text.alignment = CENTER;
		text.scale.scale(0.4);
		text.updateHitbox();
		add(text);
	...
	function createTexts()
	{
		curOptions = [];
		curOptionsValid = [];
		grpDisplay.forEachAlive(function(text:OptionFlxText) text.destroy());
		grpBlacks.forEachAlive(function(black:AttachedSprite) black.destroy());
		grpOptions.forEachAlive(function(text:OptionFlxText) text.destroy());
		grpBinds.forEachAlive(function(text:OptionFlxText) text.destroy());
		grpDisplay.clear();
		grpBlacks.clear();
		grpOptions.clear();
		grpBinds.clear();
		...
		for (i in 0...options.length)
		{
			...
			if (option[0] || onKeyboardMode)
			{
				if (option.length > 1)
				{
					...
					var text:OptionFlxText = new OptionFlxText(200, 300, option[1], 48);
					text.color = FlxColor.WHITE;
	...
	function addCenteredText(text:OptionFlxText, option:Array<Dynamic>, id:Int)
	{
		text.screenCenter(X);
		text.y -= 55;
	...
	function addKeyText(text:OptionFlxText, option:Array<Dynamic>, id:Int)
	{
		for (n in 0...2)
		{
			...
			var attach:OptionFlxText = new OptionFlxText(textX + 210, 300, key, 48);
			attach.color = FlxColor.WHITE;
	...
	/*function playstationCheck(alpha:OptionFlxText)
		{
			...
			if(model == PS4)
			{
				switch(alpha.text)
				{
					case '[', ']': //Square and Triangle respectively
						letter.image = 'OptionFlxText_playstation';
						letter.updateHitbox();
	...
	{
		var bind:OptionFlxText = grpBinds.members[num];
		var attach:OptionFlxText = new OptionFlxText(350 + (num % 2) * 300, 300, text, 48);
		attach.color = FlxColor.WHITE;
	...
	var bindingText:OptionFlxText;
	var bindingText2:OptionFlxText;
		...
		if (!binding)
		{
			...
			if (FlxG.keys.justPressed.ENTER || FlxG.gamepads.anyJustPressed(START) || FlxG.gamepads.anyJustPressed(A) #if mobile || virtualPad.buttonA.justPressed #end)
			{
				if (options[curOptions[curSelected]][1] != defaultKey)
				{
					...
					bindingText = new OptionFlxText(0, 160, "Rebinding " + options[curOptions[curSelected]][3]);
					bindingText.fieldWidth = FlxG.width;
					bindingText.alignment = CENTER;
					add(bindingText);
					
					bindingText2 = new OptionFlxText(0, 340, "Hold ESC to Cancel\nHold Backspace to Delete");
	...
	function updateText(?move:Int = 0)
	{
		...
		grpDisplay.forEachAlive(function(item:OptionFlxText) {
			item.targetY = item.ID - num - addNum;
		});
		
		grpOptions.forEachAlive(function(item:OptionFlxText) {
			item.targetY = item.ID - num - addNum;
			item.alpha = (item.ID - num == 0) ? 1 : 0.6;
		});
		grpBinds.forEachAlive(function(item:OptionFlxText) {
			var parent:OptionFlxText = grpOptions.members[item.ID];
